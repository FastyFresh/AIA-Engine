"""Transform with hair color change and background replacement"""
import asyncio
import sys
import os
...
async def transform_image(source_path: str, output_path: str, fal_key: str) -> dict:
    ...
    prompt = """Transform the person in Figure 1 to match Figure 2 (face) and Figure 3 (body).
...
- Background: Replace with spacious bright modern luxury penthouse bedroom, floor-to-ceiling windows with natural sunlight, high double-height ceilings, designer furniture, rumpled king bed with white sheets
...
- Facial expression and mood"""
    ...
    payload = {
        ...
        "negative_prompt": "black hair, dark hair, short hair, different face, different person, skeletal, plain background, simple background, white wall, studio background"
    }
    ...
    async with httpx.AsyncClient(timeout=180.0) as client:
        ...
        return {"status": "error", "error": response.text[:200] if response else "unknown"}

async def main():
    fal_key = os.getenv("FAL_KEY", "")
    
    # Test with 3 different source images
    test_images = [
        "content/transform_input/012426transform 4/SnapInsta.to_603828247_17866939128519906_3556279129994261783_n.jpg",  # dark hair
    ...
    output_dir = Path("content/transformed/starbright_monroe_v2")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    for src_path in test_images:
        src = Path(src_path)
        out = output_dir / f"v2_{src.stem}.png"
        print(f"Transforming: {src.name}")
